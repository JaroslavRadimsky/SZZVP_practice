...
def read_foreigners() -> pd.DataFrame:
...
def clean_foreigners(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cleaned["count"] = pd.to_numeric(cleaned["Hodnota"], errors="coerce").fillna(0).astype(int)
    cleaned["sex"] = cleaned["Pohlaví"].fillna("Celkem").astype(str).str.strip()
    cleaned["sex_code"] = cleaned["sex"].map({"Celkem": "TOTAL", "Muži": "M", "Ženy": "F"}).fillna(cleaned["sex"])
    cleaned["iso3"] = cleaned["country_name"].map(COUNTRY_MAP)
    cleaned["region"] = cleaned["Území"].fillna("Česko").astype(str).str.strip()
    cleaned["age_group"] = "Celkem"
    cleaned["residence_type"] = cleaned["Typ pobytu"].fillna("Celkem").astype(str).str.strip()
    return cleaned[
        ["country_name", "iso3", "year", "sex", "sex_code", "age_group", "region", "residence_type", "count"]
    ].drop_duplicates()
